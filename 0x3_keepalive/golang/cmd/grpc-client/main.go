...
var (
	...
	targetAddress      = flag.String("target-address", "", "目标地址")
...
// 处理 gRPC 订阅
func grpc_subscribe(conn *grpc.ClientConn) error {
	var err error
	...
	for {
		...
		if i == *resub {
			...
			err = stream.Send(&subscription)
			if err != nil {
				return fmt.Errorf("重新订阅失败: %v", err)
			}
		}
		...
		if err != nil {
			return fmt.Errorf("接收更新时发生错误: %v", err)
		}
